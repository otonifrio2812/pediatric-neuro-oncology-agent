...
def _dedupe_trials(trials: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Deduplicate trials by id (preferring the first occurrence)."""
    seen: set = set()
    out: List[Dict[str, Any]] = []
    for t in trials:
        key = t.get('id') or t.get('nct_id') or t.get('title')
        if not key:
            out.append(t)
            continue
...
def load_trials(path: str = 'clinical_trials/sample_trials.json') -> List[Dict[str, Any]]:
    ...
    # Dedup by id (file entries override SAMPLE_TRIALS via first-seen-wins;
    # refreshed entries dedup against earlier ones too).
