...
# Demo-resilience: a hard ceiling on each external call. The original 30s/60s
# timeouts can hang a live demo for up to 2 minutes when the network is bad.
# 20s covers normal latency comfortably while keeping the worst case bounded.
...
def pubmed_search_pmids(query: str = DEFAULT_QUERY, days: int = 30, retmax: int = 50,
    ...
    r = requests.get(PUBMED_ESEARCH, params=params, timeout=timeout)
    r.raise_for_status()
    return r.json().get('esearchresult', {}).get('idlist', [])


def pubmed_fetch_details(pmids: List[str], timeout: float = DEFAULT_TIMEOUT) -> List[Dict[str, Any]]:
    if not pmids:
        return []
    params = {
    ...
    if os.getenv('NCBI_API_KEY'):
        params['api_key'] = os.getenv('NCBI_API_KEY')
    r = requests.get(PUBMED_EFETCH, params=params, timeout=timeout)
    r.raise_for_status()
...
def clinicaltrials_search(query: str = 'pediatric diffuse midline glioma OR pediatric brain tumor',
    ...
    for _ in range(max_pages):
        ...
        r = requests.get(CTGOV_STUDIES, params=params, timeout=timeout)
        r.raise_for_status()
        data = r.json()
...
def _list_cached_rag_sources(rag_dir: str = 'rag_sources') -> Dict[str, Any]:
    """Inventory of currently-cached evidence (for --offline reporting)."""
    p = Path(rag_dir)
    inventory: Dict[str, Any] = {'rag_sources_dir': str(p), 'files': []}
    if not p.exists():
        inventory['note'] = 'rag_sources/ does not exist yet; no cached evidence.'
        return inventory
    for f in sorted(p.glob('*.jsonl')):
        try:
            line_count = sum(1 for _ in f.open('r', encoding='utf-8', errors='ignore'))
        except Exception:
            line_count = -1
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Refresh PubMed + ClinicalTrials.gov evidence for the RAG knowledge base.'
    )
    parser.add_argument('--offline', action='store_true',
                        help='Skip all network calls; preserve current rag_sources/ cache and exit.')
    parser.add_argument('--query', default=DEFAULT_QUERY, help='PubMed query string.')
    parser.add_argument('--days', type=int, default=30, help='PubMed reldate window (days).')
    parser.add_argument('--retmax', type=int, default=50, help='PubMed max hits.')
    parser.add_argument('--out-dir', default='rag_sources', help='Output dir for *.jsonl + manifest.')
    ...
    if args.offline:
        result = {
            'mode': 'offline',
            'message': (
                'Offline mode: skipped PubMed/ClinicalTrials.gov network calls. '
                'Existing rag_sources/ cache preserved; SimpleRAG will still serve it.'
            ),
            'cache': _list_cached_rag_sources(args.out_dir),
        ...
        try:
            ...
            result = {
                ...
                'error': str(exc),
                'message': 'Refresh raised unexpectedly; existing cache preserved.',
                'cache': _list_cached_rag_sources(args.out_dir),
            }
