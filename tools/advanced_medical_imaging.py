...
# ---------------------------------------------------------------------------
# SAFE_MODE: preprocessing-only path (default ON)
# ---------------------------------------------------------------------------
# Clinical safety policy: this module's segment_tumor / classify_location /
# assess_* outputs are heuristic and not validated for clinical use. The
# project's safety stance is "preprocessing + perception only — judgement
# belongs to the radiologist". Therefore SAFE_MODE is ENABLED by default and
# disables all diagnostic-adjacent computation (segmentation, volume,
# location classification, enhancement/necrosis/hemorrhage/etc.).
#
# Override (e.g. for live demo of full prototype features):
#   - Environment: IMAGING_SAFE_MODE=0
#   - Per-case:    case["imaging_safe_mode"] = False
#
# Default (production / hackathon submission): ON.

def _safe_mode_enabled(structured_case: Optional[Dict[str, Any]] = None) -> bool:
    case = structured_case or {}
    if "imaging_safe_mode" in case:
        return bool(case["imaging_safe_mode"])
    return os.getenv("IMAGING_SAFE_MODE", "1") != "0"


# ---------------------------------------------------------------------------
# DICOM PHI scrubbing
# ---------------------------------------------------------------------------
# Active scrubbing of PHI before any metadata is returned. This is in addition
# to the existing "we don't read PatientName etc." passive policy.
...
def _strip_dicom_phi(ds: Any) -> List[str]:
    """Remove PHI tags from a pydicom Dataset in-place. Returns redacted tag names.

    No-op if pydicom not available or `ds` is not a Dataset.
    """
    if pydicom is None or ds is None:
        return []
    redacted: List[str] = []
    for tag_name in PHI_DICOM_TAG_NAMES:
        try:
            if hasattr(ds, tag_name) and getattr(ds, tag_name, None):
                delattr(ds, tag_name)
                redacted.append(tag_name)
        except Exception:
            pass
    return redacted


def _scrub_string_phi(text: Any) -> str:
    """Scrub obvious PHI patterns (MRN, dates, SSN-like) from free-text DICOM strings.

    Names are NOT regex-redacted here (too many false positives on clinical terms);
    they should already be removed by _strip_dicom_phi on standard PHI tags.
    """
    if text is None:
        return ""
    s = str(text)
    if not s:
        return s
    s = re.sub(r"\bMRN[:\s]*[A-Za-z0-9_-]+", "MRN:[REDACTED]", s, flags=re.IGNORECASE)
    s = re.sub(r"\b\d{4}[-/]\d{1,2}[-/]\d{1,2}\b", "[REDACTED_DATE]", s)
    s = re.sub(r"\b\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b", "[REDACTED_DATE]", s)
    s = re.sub(r"\b\d{3}[- ]?\d{2}[- ]?\d{4}\b", "[REDACTED_SSN_LIKE]", s)
    return s
...
def _read_dicom_series(dicom_files: List[str]) -> Dict[str, SeriesVolume]:
    ...
    phi_redact_counts: Dict[str, int] = {}
    ...
    for fp in dicom_files:
        try:
            ...
            if not hasattr(ds, "PixelData"):
                continue
            # PHI scrub: remove identifying tags BEFORE we use anything from ds.
            redacted = _strip_dicom_phi(ds)
            for name in redacted:
                phi_redact_counts[name] = phi_redact_counts.get(name, 0) + 1
            ...
            continue
    if phi_redact_counts:
        warnings.warn(
            f"DICOM PHI scrub removed tags: {dict(phi_redact_counts)}",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    for uid, dss in grouped.items():
        ...
        metadata = {
            "SeriesDescription": _scrub_string_phi(getattr(first, "SeriesDescription", "")),
            "ProtocolName": _scrub_string_phi(getattr(first, "ProtocolName", "")),
            "SequenceName": _scrub_string_phi(getattr(first, "SequenceName", "")),
...
def _safe_mode_result(series: Dict[str, "SeriesVolume"]) -> Dict[str, Any]:
    """SAFE_MODE: preprocessing-only output. No segmentation/measurement/diagnosis."""
    result = {
        "status": "ok",
        "mode": "SAFE_MODE",
        ...
        # Diagnostic-adjacent fields intentionally empty in SAFE_MODE.
        "structured_findings": {},
        "cross_check": {},
        "risk_map_path": None,
        "mask_path": None,
        ...
        "safety_flags": [
            "SAFE_MODE (default ON): preprocessing only — sequence detection, spacing, metadata.",
            "Tumor segmentation DISABLED (heuristic method is not clinically validated).",
            "Volumetric measurements DISABLED.",
            "Anatomic location classification DISABLED.",
            "Automated finding assessment (enhancement/necrosis/hemorrhage/diffusion/hydrocephalus) DISABLED.",
            "Surgical risk map DISABLED.",
            "Radiologist must perform all clinical interpretation from the original study.",
            "Override (for demo of full prototype only): IMAGING_SAFE_MODE=0 or case['imaging_safe_mode']=False.",
        ],
    }
    # Custom imaging_description for SAFE_MODE (does not consume findings/location).
    lines = [
        "Structured imaging summary (SAFE_MODE — preprocessing only, AI-assisted, non-diagnostic):",
        f"- Input type: {result.get('input_type', 'DICOM/NIfTI')}",
        f"- Series detected: {', '.join(result.get('series_detected', [])) or 'none'}",
        "- SAFE_MODE policy: tumor segmentation, anatomic classification, and automated",
        "  findings (enhancement/necrosis/hemorrhage/diffusion/hydrocephalus) are intentionally DISABLED.",
        "- Clinical interpretation must be performed by a radiologist on the original imaging study.",
        f"- Safety disclaimer: {DISCLAIMER}",
        f"- Privacy note: {PRIVACY_NOTE}",
    ]
    result["imaging_description_text"] = "\n".join(lines)
...
def analyze_medical_study(
    ...
    SAFE_MODE is enabled by default — only preprocessing (sequence detection,
    metadata) is returned. See _safe_mode_enabled().
    """
    try:
        _ensure_dir(output_dir)
        series = load_study(path_or_paths)

        # Clinical-safety gate: SAFE_MODE is ON by default. Skip all diagnostic-adjacent
        # computation (segmentation, measurement, classification, findings, risk map).
        if _safe_mode_enabled(structured_case):
            return _safe_mode_result(series)

        # Full-prototype path (opt-in only via IMAGING_SAFE_MODE=0 or case flag).
        ...
        result = {
            ...
            "privacy_note": PRIVACY_NOTE,
            "safety_flags": [
                "FULL_PROTOTYPE mode (SAFE_MODE explicitly OFF).",
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", nargs="?", default=None,
                        help="DICOM folder/zip/file or NIfTI file/folder (omit when using --check-deps)")
    parser.add_argument("--output-dir", default="outputs")
    parser.add_argument("--case-json", default=None, help="Optional structured case JSON file")
    parser.add_argument("--check-deps", action="store_true",
                        help="List dependency status (nibabel/pydicom/scipy/matplotlib) and SAFE_MODE default.")
    parser.add_argument("--full-prototype", action="store_true",
                        help="DEMO ONLY: disable SAFE_MODE for this run (enables segmentation/measurement).")
    args = parser.parse_args()

    if args.check_deps:
        deps = [
            ("numpy",          True),  # imported at top; always present
            ("scipy.ndimage",  ndi is not None),
            ("nibabel",        nib is not None),
            ("pydicom",        pydicom is not None),
            ("matplotlib",     plt is not None),
        ]
        print("=== advanced_medical_imaging dependencies ===")
        for name, ok in deps:
            print(f"  [{'OK' if ok else '--'}]  {name}")
        print()
        print(f"SAFE_MODE default: {'ON' if _safe_mode_enabled() else 'OFF'}")
        print("  Override via env IMAGING_SAFE_MODE=0 or case['imaging_safe_mode']=False.")
        print(f"  When ON: preprocessing only (sequence detection + metadata, no segmentation).")
        print(f"  PHI scrub tags: {len(PHI_DICOM_TAG_NAMES)} DICOM tags actively removed.")
        sys.exit(0)

    if not args.input:
        parser.error("input path is required unless --check-deps is used")

    case: Optional[Dict[str, Any]] = None
    if args.case_json:
        case = json.loads(Path(args.case_json).read_text(encoding="utf-8"))
    if args.full_prototype:
        case = dict(case or {})
        case["imaging_safe_mode"] = False
