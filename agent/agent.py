...
""".strip()
    ...
    def run(self, raw_case_text: str, structured: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        drug_md = ''
        # Drug-ranking module is now ENABLED BY DEFAULT (Cellosaurus-verified, Cellosaurus-audited).
        # It gracefully degrades when artifacts are missing — see drug_ranking_adapter.rank_drugs_for_case.
        # Set ENABLE_DRUG_RANKING=0 to force-disable (e.g. CI without network).
        if os.getenv('ENABLE_DRUG_RANKING', '1') != '0' or case.get('enable_drug_ranking'):
            try:
    ...
    def compose_report(self, raw_case_text: str, case: Dict[str, Any], missing: List[str], evidence: List[Dict[str, Any]], reasoning: str, trials_md: str, drug_md: str, trace: List[str]) -> str:
        ...
        # Optional: Agent Architecture appendix (graceful skip if no image asset found).
        # Drop an image at assets/architecture.png (or 腦瘤架構圖.jpg / architecture.jpg /
        # brain_tumor_architecture.jpg) to enable. The whole block is wrapped in try/except —
        # missing module, missing image, or copy error all degrade silently to no section.
        try:
            from architecture_report_integration import install_architecture_asset, architecture_markdown
            arch = install_architecture_asset()
            if arch.get('status') == 'ok' and arch.get('output_path'):
                lines.append(architecture_markdown(arch['output_path']))
                lines.append('')
        except Exception:
            pass  # never break the report on architecture failure
